...
def main():
    ...
    directory = "Data/del_signals_features_random_with_gaussian"
    ...
    if os.path.exists(directory):
        ...
        for filename in file_list:
            ...
            if not ((filename.startswith("signals_features_random_with_gaussian_plot") and filename.endswith(".png")) or
                     (filename.startswith("signals_features_random_with_gaussian") and filename.endswith(".csv"))):
                # Delete the file
                try:
                    os.remove(file_path)
    ...
    features_dict = {
        "sigma_001": ['rand_with_gaussian_sigma_1'],
        "sigma_070": ['rand_with_gaussian_sigma_70'],
        "sigma_035": ['rand_with_gaussian_sigma_35'],
        "sigma_105": ['rand_with_gaussian_sigma_105'],
        "sigma_140": ['rand_with_gaussian_sigma_140'],
    }
    ...
    supervised_parameters = {
        'model_path_sigma_001': 'supervised/models/interp_rand_with_gaussian/interp/n_epochs-600_LSTM_dropout-0.5_hidden-zeros/interp_rand_with_gaussian_interp_rand_with_gaussian_sigma_001/',
        'model_path_sigma_035': 'supervised/models/interp_rand_with_gaussian/interp/n_epochs-600_LSTM_dropout-0.5_hidden-zeros/interp_rand_with_gaussian_interp_rand_with_gaussian_sigma_035/',
        'model_path_sigma_070': 'supervised/models/interp_rand_with_gaussian/interp/n_epochs-600_LSTM_dropout-0.5_hidden-zeros/interp_rand_with_gaussian_interp_rand_with_gaussian_sigma_070/',
        'model_path_sigma_105': 'supervised/models/interp_rand_with_gaussian/interp/n_epochs-600_LSTM_dropout-0.5_hidden-zeros/interp_rand_with_gaussian_interp_rand_with_gaussian_sigma_105/',
        'model_path_sigma_140': 'supervised/models/interp_rand_with_gaussian/interp/n_epochs-600_LSTM_dropout-0.5_hidden-zeros/interp_rand_with_gaussian_interp_rand_with_gaussian_sigma_140/',
    ...
    signals_count = 50  # for averaging
    freqs = (1, 12)
    ...
    ax.set_title(f'{str(error_type).capitalize()} Average by random with gaussian filter and Sampling Methods\nWith linear interpolation')
    ax.set_xticks(x)
    ...
    file_path_csv = directory + f"/signals_features_random_with_gaussian_{str(error_type)}.csv"
    counter = 1
    while os.path.exists(file_path_csv):
        file_path_csv = directory + f"/signals_features_random_with_gaussian_{str(error_type)}_{counter}.csv"
        counter += 1

    pd.DataFrame(features_mse_avg_mat, index=x_labels).to_csv(file_path_csv, header=sampling_methods, index=True)

    # Define the file path and check if it already exists
    file_path = directory + "/signals_features_random_with_gaussian_plot.png"
    counter = 1
    while os.path.exists(file_path):
        file_path = directory + f"/signals_features_random_with_gaussian_plot_{counter}.png"
        counter += 1
